...
import { PaymentStatus, PaymentMethod, PaymentPlan, OrderStatus, Installment } from '@prisma/client';
...
export class PaymentsService {
  ...
  async confirmPayment(orderId: string) {
    ...
    if (payment.paymentPlan === PaymentPlan.FULL) {
      ...
      // Confirm the order since first installment is paid
  ...
  async getInstallmentSchedule(orderId: string) {
    const payment = await this.prisma.payment.findUnique({
      where: { orderId },
      include: {
        installments: {
