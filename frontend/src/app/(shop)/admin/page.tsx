    ...
    <div className="container" style={{ padding: '2rem 0 4rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        {[
          ...
          { label: 'Revenue', value: `$${stats.totalRevenue.toFixed(2)}`, icon: '💰', color: '#ecfdf5' },
              ...
              <tbody>
                {orders.map((order) => (
                  <tr key={order.id} style={{ borderBottom: '1px solid var(--color-border-light)' }}>
                    ...
                    <td style={{ padding: '0.75rem 1rem', fontWeight: 600 }}>${Number(order.total).toFixed(2)}</td>
            ...
            <tbody>
              {products.map((p) => (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--color-border-light)' }}>
                  ...
                  <td style={{ padding: '0.75rem 1rem', fontWeight: 600 }}>${Number(p.price).toFixed(2)}</td>
                  <td style={{ padding: '0.75rem 1rem' }}>
